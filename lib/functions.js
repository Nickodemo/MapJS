...
*/
	...
	function getColor(Usos_suelo) { 
		return  Usos_suelo === "Terrenos con escasa o nula vegetacion" ? '#FFDEAD' : 
				Usos_suelo === "Salinas" ? '#87CEFA' : 
				Usos_suelo === "Pastizal" ? '#90EE90' : 
				Usos_suelo === "Masas de agua" ? '#AFEEEE' : 
	...
	function getUsosComarca1987(){
		...
		var area = area_interseca_usos(Usos_suelo_1987,Comarca_1987);
		char_data = getAreaUsos(area);
		
		if (map1987.isSynced()) {	//Si el map1987 está sincronizado						
			area = area_interseca_usos(Usos_suelo_2014,Comarca_1987);
			char_data2014 = getAreaUsos(area);
		}
		//Se crea pop up
		$("#info_modal").click();
		$("#info_prov").html('La superficie del poligono seleccionado es: <b>'+(turf.area(Comarca_1987)/1000000).toFixed(2)+'</b> km<sup>2</sup>');
	}
	
	function getUsosComarca2014(){
		
		var area = area_interseca_usos(Usos_suelo_2014,Comarca_2014);
		char_data = getAreaUsos(area);
		
		if (map1987.isSynced()) {	//Si el map1987 está sincronizado						
			area = area_interseca_usos(Usos_suelo_1987,Comarca_2014);
			char_data2014 = getAreaUsos(area);
		}
		//Se crea pop up
		$("#info_modal").click();
		$("#info_prov").html('La superficie del poligono seleccionado es: <b>'+(turf.area(Comarca_2014)/1000000).toFixed(2)+'</b> km<sup>2</sup>');
	}
	...
	function geoprocesos1987(e){
		...
		if (type === 'marker') {
			...
			$("#info_modal").click();
			$("#info_prov").html('La distancia de la línea es <b>'+dist_polyline+'</b> km<sup>2</sup>');
			//Se crea popup
		}
