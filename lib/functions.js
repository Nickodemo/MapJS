		...
		if (type === 'marker') {
			...
			if (map2014.isSynced()) {	//Si el map2014 está sincronizado
				...
				for( i=0; i<Usos_suelo_1987.features.length; i++ ){
					within = turf.inside(point_aux, Usos_suelo_1987.features[i]);				
					if (within){//Si within es true, entra
						marker1987.bindPopup('Cobertura: '+ Usos_suelo_1987.features[i].properties.Usos_suelo+' <br>Coordenadas: ('+point_aux.geometry.coordinates[1].toFixed(2)+','+point_aux.geometry.coordinates[0].toFixed(2)+') <br>Municipio: '+str_municipio);
					}
				}
			}	
			...
			if (map2014.isSynced()) {	//Si el map1987 está sincronizado						
				...
				var poligono1987=[]; //Crea el poligono que almacenará las coordenadas de e
				for(i=0; i<e.layer._latlngs.length; i++){
					poligono1987[i]=e.layer._latlngs[i];
				}
				L.polygon(poligono1987).addTo(map1987); //Dibuja 'e' en map2014
				
				area = area_interseca_usos(Usos_suelo_1987,polygon_aux);
				char_data2 = [(area[0]/1000000).toFixed(2),(area[1]/1000000).toFixed(2),(area[2]/1000000).toFixed(2),(area[3]/1000000).toFixed(2),(area[4]/1000000).toFixed(2),(area[5]/1000000).toFixed(2),(area[6]/1000000).toFixed(2),(area[7]/1000000).toFixed(2)];
			}
