	...
	function geoprocesos1987(e){
		...
		if (type === 'marker') {
			...
			for( i=0; i<Usos_suelo_1987.features.length; i++ ){
				...
				if (within){//Si within es true, entra
					layer.bindPopup('Cobertura: '+ Usos_suelo_1987.features[i].properties.Usos_suelo+' <br>Coordenadas: ('+point_aux.geometry.coordinates[1].toFixed(4)+','+point_aux.geometry.coordinates[0].toFixed(4)+') <br>Municipio: '+str_municipio);
				}
			}
			...
			if (map1987.isSynced()) {	//Si el map1987 está sincronizado
				...
				for( i=0; i<Usos_suelo_2014.features.length; i++ ){
					...
					if (within){//Si within es true, entra
						marker2014.bindPopup('Cobertura: '+ Usos_suelo_2014.features[i].properties.Usos_suelo+' <br>Coordenadas: ('+point_aux.geometry.coordinates[1].toFixed(4)+','+point_aux.geometry.coordinates[0].toFixed(4)+') <br>Municipio: '+str_municipio);
					}
				}
			}
	...
	function geoprocesos2014(e){
		...
		if (type === 'marker') {
			...
			if (map2014.isSynced()) {	//Si el map1987 está sincronizado
				polygon_aux.addTo(map1987); //Dibuja 'e' en map2014
			...
			if (map2014.isSynced()) {	//Si el map1987 está sincronizado				
				L.circle(e.layer._latlng,layer._mRadius).addTo(map1987); //Dibuja 'e' en map2014
