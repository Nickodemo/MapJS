...
*/
	...
	function geoprocesos1987(e){
		...
		if (type === 'marker') {
			...
			layer.bindPopup('Cobertura: '+ getUso1987(layer) +' <br>Coordenadas: ('+point_aux.geometry.coordinates[1].toFixed(4)+','+point_aux.geometry.coordinates[0].toFixed(4)+') <br>Municipio: '+getMunicipio(layer));

			if (map1987.isSynced()) {	//Si el map1987 está sincronizado				
				var marker2014= new L.marker(layer._latlng).addTo(map2014);
				marker2014.bindPopup('Cobertura: '+ getUso2014(layer) +' <br>Coordenadas: ('+point_aux.geometry.coordinates[1].toFixed(4)+','+point_aux.geometry.coordinates[0].toFixed(4)+') <br>Municipio: '+getMunicipio(layer));
			}
			...
			$("#info_modal").click();
			if (map1987.isSynced()) {	//Si el map1987 está sincronizado	
				$("#info_prov").text('La superficie de las areas es '+char_data+' '+char_data2014);
			}else{
				$("#info_prov").text('La superficie de las areas es '+char_data);
			}			
			...
			if (map1987.isSynced()) {	//Si el map1987 está sincronizado	
				...
				if($("#tab_1987").attr("class")==="active"){
					$("#info_prov").text('La superficie de las areas es '+char_data);
				}else{
					$("#info_prov").text('La superficie de las areas es '+char_data2014);
				}
			}
	...
	function geoprocesos2014(e){
		...
		if (type === 'marker') {
			var point_aux=new turf.point([layer._latlng.lng,layer._latlng.lat]);				
			layer.bindPopup('Cobertura: '+ getUso2014(layer) +' <br>Coordenadas: ('+point_aux.geometry.coordinates[1].toFixed(4)+','+point_aux.geometry.coordinates[0].toFixed(4)+') <br>Municipio: '+getMunicipio(layer));

			if (map2014.isSynced()) {	//Si el map2014 está sincronizado
				var marker1987= new L.marker(layer._latlng).addTo(map1987);
				marker1987.bindPopup('Cobertura: '+ getUso1987(layer) +' <br>Coordenadas: ('+point_aux.geometry.coordinates[1].toFixed(4)+','+point_aux.geometry.coordinates[0].toFixed(4)+') <br>Municipio: '+getMunicipio(layer));
			}
